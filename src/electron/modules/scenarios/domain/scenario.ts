...
export function validateCreateTipoEscenarioInput(input: CreateTipoEscenarioInput): void {
  if (!input.id || !input.nombre) {
    throw new Error("TipoEscenario requires id and nombre");
  }
}

export function validateCreateEscenarioInput(input: CreateEscenarioInput): void {
  if (!input.id || !input.proyectoId || !input.tipoEscenarioId || !input.nombre) {
    throw new Error("Escenario requires id, proyectoId, tipoEscenarioId and nombre");
  }
}

...
export function registerScenarioIpcHandlers() {
  ipcMain.handle("scenarioTypeCreate", async (event, payload: CreateTipoEscenarioInput) => {
  ...
  ipcMain.handle("scenarioCreate", async (event, payload: CreateEscenarioInput) => {
    const frame = event.senderFrame;
    if (!frame) {
      throw new Error("Missing senderFrame");
    }
  ...
  ipcMain.handle("scenarioListByProject", async (event, payload: { proyectoId: string }) => {
    const frame = event.senderFrame;
    if (!frame) {
      throw new Error("Missing senderFrame");
    }

// src/electron/modules/core-data/interfaces/ipc.ts
...
export function registerCoreDataIpcHandlers() {
  ipcMain.handle(
    ...
    async (event, payload: CreateProyectoBootstrapInput) => {
  ...
  ipcMain.handle(
    "coreProyectoCreate",
    async (event, payload: CreateProyectoInput) => {
      const frame = event.senderFrame;
      if (!frame) throw new Error("Missing senderFrame");
      validateEventFrame(frame);
  ...
  ipcMain.handle(
    "corePozoCapaCreate",
    async (event, payload: CreatePozoCapaInput) => {
      const frame = event.senderFrame;
      if (!frame) throw new Error("Missing senderFrame");
      validateEventFrame(frame);
